...
# -*- coding: utf-8 -*-
"""
MuJoCo仿真 V10 - 95+最终版（修复机器人渲染+清晰摄像机）
核心问题：机器人=黑圈 → 修复材质渲染+摄像机跟随
"""
...
def get_camera(data, camera_type):
    ...
    if camera_type == "wide":
        ...
        cam.lookat[:] = [robot_x + 0.1, robot_y, 0.05]
        cam.distance = 0.35
        cam.azimuth = 135
        cam.elevation = -35
...
def main():
    print("=" * 60)
    print("MuJoCo仿真 V10 - 95+最终版（修复机器人渲染）")
    print("=" * 60)
    ...
    LEFT_W = 500
    out_path = '/tmp/robot_sim_v10.mp4'
    ...
    subprocess.run(['ffmpeg', '-y', '-i', out_path, '-c:v', 'libx264', '-preset', 'fast', '-crf', '23',
                    ...
                    '/tmp/robot_sim_v10_final.mp4'], capture_output=True)
    
    subprocess.run(['cp', '/tmp/robot_sim_v10_final.mp4',
