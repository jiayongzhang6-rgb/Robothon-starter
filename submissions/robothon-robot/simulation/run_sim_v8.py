...
def main():
    ...
    for frame_idx in range(total_frames):
        ...
        # 更好的相机角度 - 侧上方视角，能看到机器人
        cam = mujoco.MjvCamera()
        cam.type = mujoco.mjtCamera.mjCAMERA_FREE
        cam.lookat[:] = [data.qpos[0], 0, 0.05]
        cam.distance = 0.35  # 更近
        cam.azimuth = 135  # 45度角
        cam.elevation = -30  # 俯视
    ...
    subprocess.run(['ffmpeg', '-y', '-i', out_path, '-c:v', 'libx264', '-preset', 'fast', '-crf', '23',
                    ...
                    '/tmp/robot_sim_v8_final.mp4'], capture_output=True)
    
    subprocess.run(['cp', '/tmp/robot_sim_v8_final.mp4',
