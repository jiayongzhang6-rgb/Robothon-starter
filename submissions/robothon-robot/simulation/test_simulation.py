...
def test_obstacle_avoidance():
    ...
    for i in range(1500):
        ...
        if data.qpos[0] > 0.2 and data.qpos[0] < 0.25:
...
def test_endpoint_localization():
    ...
    print(f"  是否到达终点: {'是' if final_x > 0.4 else '否'}")
    return final_x > 0.4
...
def test_pid_stability():
    ...
    print(f"  稳定性: {'稳定' if error_std < 0.1 else '不稳定'}")
    return error_std < 0.1

def main():
    """运行所有测试"""
    print("=" * 60)
    print("MuJoCo仿真测试 - 真实物理测试")
    print("=" * 60)
