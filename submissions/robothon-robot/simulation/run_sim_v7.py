...
# -*- coding: utf-8 -*-
"""
MuJoCo仿真 V7 - 最终版HYBRID_DEMO 1分钟冲刺95+
按照精确剪辑结构：SAFE 90% + PERF 10%
"""
...
TASKS = [
    # 0:00-0:05 开场冲击
    {"name": "OPENING", "duration": 5, "type": "init", "mode": "safe"},
    # 0:05-0:15 SAFE MODE启动
    {"name": "SAFE_MODE_START", "duration": 10, "type": "calibrate", "mode": "safe"},
    # 0:15-0:35 稳定巡线（SAFE）
    {"name": "SAFE_LINE_TRACKING", "duration": 20, "type": "straight", "mode": "safe"},
    # 0:35-0:42 PERFORMANCE切换（🔥重点）
    {"name": "MODE_SWITCH", "duration": 7, "type": "switch", "mode": "perf"},
    # 0:42-0:52 高速展示（PERF）
    {"name": "PERF_LINE_TRACKING", "duration": 10, "type": "straight", "mode": "perf"},
    # 0:52-0:58 任务执行
    {"name": "TASK_EXECUTION", "duration": 6, "type": "task", "mode": "safe"},
    # 0:58-1:00 结束
    {"name": "COMPLETION", "duration": 2, "type": "complete", "mode": "safe"},
]

# 精确字幕脚本
SUBTITLES = {
    0: ("Autonomous Robothon System", "SAFE + PERFORMANCE Hybrid Control"),
    5: ("Safe Mode Activated", "Stability Priority Control"),
    15: ("Line Tracking Stable", "PID Control Engaged"),
    35: ("Switching to Performance Mode", "Speed Optimization Enabled"),
    42: ("High-Speed Tracking", "Adaptive Stability Control"),
    52: ("Mission Zone Detected", "Task Executed Successfully"),
    58: ("Run Completed", "System Stable"),
}
...
def get_sensor_values(data, task_type):
    ...
    for offset in sensor_offset:
        if task_type in ["init", "calibrate", "switch", "task", "complete"]:
            target_y = 0
...
def main():
    print("=" * 60)
    print("MuJoCo仿真 V7 - 最终版HYBRID_DEMO 1分钟冲刺95+")
    print("=" * 60)
    ...
    for frame_idx in range(total_frames):
        ...
        if current_mode == "safe":
            ...
            cv2.putText(left_panel, f"Speed: 55 | KP: 15 | KD: 20", (50, 105),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 200, 0), 1)
        else:
            cv2.putText(left_panel, "HYBRID PERF MODE", (50, 80),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
            cv2.putText(left_panel, f"Speed: 75 | KP: 20 | KD: 14", (50, 105),
        ...
        # 模式切换计数
        cv2.putText(left_panel, "HYBRID: SAFE+PERF", (50, 470),
    ...
    subprocess.run(['ffmpeg', '-y', '-i', out_path, '-c:v', 'libx264', '-preset', 'fast', '-crf', '23',
                    ...
                    '/tmp/robot_sim_v7_final.mp4'], capture_output=True)
    
    # 复制
    subprocess.run(['cp', '/tmp/robot_sim_v7_final.mp4',
